...
/**
 *  Created by madong on 2024/9/03.
 */
object SimpleRDDJoins {

  val spark = SparkSession.builder().appName("RDD joins")
    .master("local[*]")
    .getOrCreate()
  val sc = spark.sparkContext

  val rootFolder = "example-data/rtjvm/exam_data"
//  Generator.generateExamData(rootFolder, 1000000, 5)
  ...
  def coPartitioning() = {
    ...
    // 不会产生shuffle，应为candidates和scores的partitioner一样，分区数也一样
